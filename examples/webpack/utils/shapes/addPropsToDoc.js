...
import { drawCustomRect, drawCustomSVG, drawCircle } from "./helper.js";
...
async function addImage(item, doc) {
  try {
    ...
    ctx.drawImage(canvas, -imgCropX, -imgCropY);
    ...
    doc.save();
    doc.clip();
    doc.image(pngBuffer, item.x, item.y, {
      width: item.width,
      height: item.height,
    });
    doc.restore();
...
export {
  ...
  addOpacity,
  addOverlayColor,
  addOverlayColorStarRating,
  addOverlayColorImage,
};
