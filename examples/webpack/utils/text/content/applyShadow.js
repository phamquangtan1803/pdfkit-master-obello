export function applyShadow(doc, textContent, line, linePosY) {
  ...
  if (textContent.shadowBlur > 0 && textContent.shadowColor) {
    const shadowPaddingX =
      Math.abs(textContent.shadowOffsetX || 0) + textContent.shadowBlur;
    const shadowPaddingY =
      Math.abs(textContent.shadowOffsetY || 0) + textContent.shadowBlur;
    const canvasWidth =
      (textContent.autoFitBackgroundEnabled
        ? textContent.width
    ...
    doc.image(
      ...
      textContent.posX - shadowPaddingX,
      linePosY - shadowPaddingY,
      {
        width: canvasWidth,
        height: canvasHeight,
      }
